...
* **Рабочие программы дисциплин (РПД)**: Полный конвейер — LLM-генерация черновика, импорт из .docx/PDF, согласование (draft→на согласовании→утверждена), ревизии, актуализация по приказам Минобрнауки, ручное редактирование всех коллекций (цели, компетенции, темы, ФОС, литература)
* **ИИ-ассистент «Методыч»**: Семантический поиск по документам (RAG на pgvector) и генерация содержимого
...
* **Электронная подпись документов**: Криптографическая ЭЦП версии документа (ECDSA P-256 + самоподписанный X.509, приватный ключ зашифрован AES-256-GCM at-rest) с проверкой целостности — детектирует изменение тела документа после подписи
* **Учёт долгов студентов**: Реестр академических задолженностей (пересдачи/комиссия), импорт из Excel и 1С (OData)
...
* **Мультиканальные уведомления**: In-app, Email, Telegram, Slack и Web Push (VAPID)
* **Многофакторная аутентификация (MFA)**: TOTP с шифрованием секрета at-rest (AES-256-GCM)
...
- `frontend/` - Frontend на Next.js 16 с Turbopack
- `docs/` - Документация проекта
- `migrations/` - SQL миграции базы данных (49 миграций)
...
- **Фреймворк**: Next.js 16 + React 19 + TypeScript 5.7 + Turbopack
- **UI библиотека**: Radix UI + Tailwind CSS 4
- **Управление состоянием**: Zustand 5
- **Загрузка данных**: SWR 2.3 + axios 1.16
...
**Подробности**: См. [docs/architecture/LOGGING_AND_PERFORMANCE.md](docs/architecture/LOGGING_AND_PERFORMANCE.md)
...
**Стек технологий (обновлено 2026-06-14)**:
- Backend: Go 1.25 • PostgreSQL 17 (pgvector) • Redis 7 • MinIO • Gin • DDD • Clean Architecture • OpenTelemetry
- Frontend: Next.js 16 • React 19 • TypeScript 5.7 • Tailwind 4 • Zustand 5 • Turbopack

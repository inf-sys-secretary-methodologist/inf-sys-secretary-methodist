...
func TestNewApproveRevisionUseCase_PanicsOnNilRepo(t *testing.T) {
	defer func() {
		if r := recover(); r == nil {
			t.Fatal("expected panic on nil repo")
		}
	}()
	NewApproveRevisionUseCase(nil, nil)
}

func TestApproveRevisionUseCase_HappyPath(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			uc := NewApproveRevisionUseCase(repo, audit)

			got, err := uc.Execute(context.Background(), tc.actorID, tc.role,
				ApproveRevisionInput{WorkProgramID: 100, RevisionID: 500})
			require.NoError(t, err)
			require.NotNil(t, got)

			rev := got.Revisions()[0]
			assert.Equal(t, domain.RevisionStatusApproved, rev.Status())
			require.NotNil(t, rev.ApproverID())
			assert.Equal(t, tc.actorID, *rev.ApproverID())
			assert.Equal(t, 1, repo.updateCalls)
			require.Len(t, audit.events, 1)
			assert.Equal(t, "work_program.revision_approved", audit.events[0].Action)
			assert.Equal(t, int64(500), audit.events[0].Fields["revision_id"])
		})
	}
}

func TestApproveRevisionUseCase_ForbiddenRole(t *testing.T) {
	for _, role := range []string{"teacher", "academic_secretary", "student", ""} {
		t.Run(role, func(t *testing.T) {
			...
			require.Len(t, audit.events, 1)
			assert.Equal(t, "work_program.revision_approve_denied", audit.events[0].Action)
			assert.Equal(t, "forbidden_role", audit.events[0].Fields["reason"])
		})
	}
}

func TestApproveRevisionUseCase_NotFoundPropagates(t *testing.T) {
...
func TestApproveRevisionUseCase_RevisionNotFound(t *testing.T) {
	...
	_, err := uc.Execute(context.Background(), 55, "methodist",
		ApproveRevisionInput{WorkProgramID: 100, RevisionID: 999})
...
func TestApproveRevisionUseCase_WrongStatusRejected(t *testing.T) {
...
func TestApproveRevisionUseCase_UpdateErrorPropagates(t *testing.T) {
	wp := approvedWPWithRevisionAt(t, 100, 7, 500, domain.RevisionStatusPendingApproval)
	repo := &fakeTransitionRepo{wp: wp, updateErr: repositories.ErrWorkProgramVersionConflict}
	uc := NewApproveRevisionUseCase(repo, &recordingAuditSink{})
...
func TestApproveRevisionUseCase_NilSinkTolerated(t *testing.T) {
	wp := approvedWPWithRevisionAt(t, 100, 7, 500, domain.RevisionStatusPendingApproval)
	repo := &fakeTransitionRepo{wp: wp}
	uc := NewApproveRevisionUseCase(repo, nil)

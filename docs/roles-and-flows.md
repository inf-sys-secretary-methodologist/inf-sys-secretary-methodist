...
> **Версия проекта:** 0.119.0 (см. `VERSION` в корне)
> **Состояние на:** 6 мая 2026 (после релизов password recovery, n8n absence-alert, Document.Update ownership, teacher analytics scope filter, Assignments bounded context, Assignments read-side + grading UI, Returned transition flow, Student resubmit flow [backend], Student read-side endpoints, Student My Assignments page, Student Resubmit UI — academic loop замкнут end-to-end в UI, Curriculum module backend [v0.116.0+v0.117.0] — author→approve loop замкнут backend, Curriculum frontend list page [v0.118.0], Curriculum detail page + edit dialog + Submit dialog [v0.119.0] — methodist self-edit cycle закрыт)

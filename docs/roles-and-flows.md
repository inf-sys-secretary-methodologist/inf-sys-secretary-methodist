...
> **Версия проекта:** 0.112.0 (см. `VERSION` в корне)
> **Состояние на:** 4 мая 2026 (после релизов password recovery, n8n absence-alert, Document.Update ownership, teacher analytics scope filter, Assignments bounded context, Assignments read-side + grading UI, Returned transition flow, Student resubmit flow [backend])
...
10. **Resubmit на assignments (только backend, 0.112.0):** при наличии student dashboard в будущем студент сможет пересдать работу, возвращённую преподавателем (`POST /api/assignments/:id/resubmit` — endpoint уже есть, UI отложен до student-dashboard initiative)
11. *(Личные настройки — стандартно для всех ролей)*

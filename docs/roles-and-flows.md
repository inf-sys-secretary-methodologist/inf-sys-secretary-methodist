...
> **Версия проекта:** 0.160.0 (см. `VERSION` в корне)
> **Состояние на:** 23 мая 2026 — **v0.160.0 users Tier 1 security hotfix shipped** (Profile takeover authorize / Departments+Positions CRUD admin gate / Avatar prefix-validate / DeleteUser self+last-admin guards + UpdateUserStatus parity). Batch 1 audit + curriculum role swap closed; batch 2 hotfixes **2/5 closed** (auth ✅ v0.159.0, users ✅ v0.160.0 — files/messaging/announcements v0.161.0..v0.163.0 pending); v0.160.1 polish patch tracked for DIP relocation + AuditSink narrow port + 5 other Tier 2 items; Phase 6 #196 strict > 90% coverage ✅; 5-phase Documents workflow pack #227 closed end-to-end; #41 Workflow automation closed; Phase 5 admin observability closed; B-feature triad closed (curriculum + assignments + B4 annual report); MFA полностью end-to-end UI.

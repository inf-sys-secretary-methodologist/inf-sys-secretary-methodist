...
> **Версия проекта:** 0.114.0 (см. `VERSION` в корне)
> **Состояние на:** 6 мая 2026 (после релизов password recovery, n8n absence-alert, Document.Update ownership, teacher analytics scope filter, Assignments bounded context, Assignments read-side + grading UI, Returned transition flow, Student resubmit flow [backend], Student read-side endpoints, Student My Assignments page)

...
> **Версия проекта:** 0.159.0 (см. `VERSION` в корне)
> **Состояние на:** 21 мая 2026 — **v0.159.0 auth Tier 1 security hotfix shipped** (revocation bypass + refresh rotation §10.4 cascade + brute-force lockout + MFA AES-GCM at-rest + reset token sha256 + GETDEL atomic consume). Batch 1 audit + curriculum role swap closed; batch 2 hotfixes 1/5 closed (auth ✅ — users/files/messaging/announcements v0.160.0..v0.163.0 pending); Phase 6 #196 strict > 90% coverage ✅; 5-phase Documents workflow pack #227 closed end-to-end; #41 Workflow automation closed; Phase 5 admin observability closed; B-feature triad closed (curriculum + assignments + B4 annual report); MFA полностью end-to-end UI.

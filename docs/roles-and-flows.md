...
> **Версия проекта:** 0.111.0 (см. `VERSION` в корне)
> **Состояние на:** 4 мая 2026 (после релизов password recovery, n8n absence-alert, Document.Update ownership, teacher analytics scope filter, Assignments bounded context, Assignments read-side + grading UI, Returned transition flow)

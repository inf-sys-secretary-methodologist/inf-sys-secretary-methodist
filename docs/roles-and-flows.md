...
> **Версия проекта:** 0.113.0 (см. `VERSION` в корне)
> **Состояние на:** 6 мая 2026 (после релизов password recovery, n8n absence-alert, Document.Update ownership, teacher analytics scope filter, Assignments bounded context, Assignments read-side + grading UI, Returned transition flow, Student resubmit flow [backend], Student read-side endpoints)
...
10. **Мои работы (только backend, 0.113.0):** GET `/api/assignments/my` (список своих submission'ов с фильтром по статусу) + GET `/api/assignments/:id/my` (детальный view) — за `RequireRole("student")`. UI ждёт v0.114.0.
11. **Resubmit на assignments (только backend, 0.112.0):** при наличии student dashboard в будущем студент сможет пересдать работу, возвращённую преподавателем (`POST /api/assignments/:id/resubmit` — endpoint уже есть, UI отложен до v0.115.0)

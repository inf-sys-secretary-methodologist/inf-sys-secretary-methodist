...
| **schedule** (расписание + lessons + teaching_load + автогенерация) | ~5800 | `/schedule` timetable grid + `/calendar` events + week-type табы + `/teaching-load` (нагрузка) + **`/schedule/generate` автогенерация (CSP-солвер, preview→apply, #139)** | ✅ |
...
> **Закрыто:** ~~Автогенерация расписания [#139]~~ (v0.222.0, CSP-солвер на чистом Go + preview/apply + UI) · ~~Электронная подпись [#140]~~ (v0.221.0, ЭЦП ECDSA P-256 + X.509) · ~~Внешние календари [#40]~~ (v0.222.0, iCalendar-фид подписки — Google/Outlook/Apple, in-house). **После #139 бэклог GitHub-issue пуст — все продуктовые issue закрыты E2E** (осталась только TM-заметка про Web Speech API).

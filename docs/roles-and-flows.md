...
> **Версия проекта:** 0.125.1 (см. `VERSION` в корне)
> **Состояние на:** 8 мая 2026 (после релизов password recovery, n8n absence-alert, Document.Update ownership, teacher analytics scope filter, Assignments bounded context + UI, Returned transition flow, Student resubmit + read endpoints + UI, Curriculum module backend [v0.116.0+v0.117.0], Curriculum frontend list page [v0.118.0], Curriculum detail+edit+submit dialogs [v0.119.0], Curriculum admin approve queue + Approve/Reject dialogs [v0.120.0], MFA TOTP enrollment for system_admin [v0.124.0 — RFC 6238 self-implemented], CI lint cleanup [v0.124.1 — misspell + prettier patch], **Login-flow MFA gating backend [v0.125.0 — intermediate token + verify-login endpoint, frontend deferred к v0.125.1]** — **defence-ready минимум закрыт 100%, curriculum end-to-end UI clickable, admin MFA enrollment visible, login-flow gating active backend-side**)
